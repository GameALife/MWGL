...
- `nodes` 表示流程中的状态或动作（如开始、分支、循环、成功/失败终态）
- `edges` 表示状态转移关系，必要时用 `label` 承载条件语义
- 通过统一类型系统（`start`、`switch`、`parallel`、`loop_start`、`loop_end` 等）描述控制流结构
...
本项目（MWGL Studio v2）是 MWGL v2 的工程化工作台：负责编辑、生成、校验、优化与代码落地。
