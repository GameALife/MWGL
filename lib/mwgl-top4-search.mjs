/**
 * Top-4 搜索（束搜索 / MCTS 二选一）：
 * - 第 0 轮：DeepSeek 并行初池 → 保留 top4
 * - 束搜索（默认）：每轮对 top4 **全部**父代并行 2 次 Qwen（mutation_focus=content|structure）→ 子代取 top4
 * - MCTS：UCT 选父代再扩两路；轮数 = top4_rounds + top4_mcts_extra_rounds
 * - 全程 globalBest
 */
...
export async function runTop4Search({
  ...
  const keep = Math.max(1, Math.floor(config.top4_keep || 4));
  ...
  const initialPool = Math.max(keep, Math.floor(config.top4_initial_pool || 8));
  ...
  async function judgeWorkflow(workflow, meta) {
    const judged = await evaluateCandidate(workflow, evalDataset, config.weights, context);
  ...
  function syncPoolFromTree() {
    const nodes = collectScoredNodes(mctsRoot);
    pool = dedupeBySignature(nodes.map(mctsNodeToCandidate))
      .sort((a, b) => b.score - a.score)
      .slice(0, keep);
  }

  function attachPoolToMctsRoot() {
  ...
  const needGen = Math.max(0, initialPool - pool.length);
  if (needGen > 0) {
    const temps = temperaturesForCount(needGen);
    const results = await Promise.all(temps.map((t) => generateWorkflowFromPrompt(prompt, t)));
  ...
  pool = dedupeBySignature(pool)
    .sort((a, b) => b.score - a.score)
    .slice(0, keep);

  if (context.graphEditEval?.enabled && !context.graphEditEval.referenceWorkflow && pool.length > 0) {
    context.graphEditEval.referenceWorkflow = pool[0].workflow;
  }

  if (searchMode === "mcts") {
    attachPoolToMctsRoot();
  }
  ...
  return {
    ...
    history,
    dropped,
    stoppedEarly: pool.length === 0 && !globalBest,
    search_mode: searchMode,
    mcts:
      searchMode === "mcts"
        ? {
