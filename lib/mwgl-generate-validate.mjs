...
export const MWGL_V3_GENERATE_SYSTEM_PROMPT =
  ...
  "节点 type 仅：start | step | branch | parallel | end。\n" +
  "- start：唯一入口，无入边，≥1 出边。\n" +
  "- step：顺序动作，最多 1 条出边；文案用「动词+对象」。\n" +
  "- branch：条件分支（择一执行），≥2 出边；每条出边 label 非空且不重复、有业务语义（禁纯数字/分支N）。二选一、if/else、满足条件才走等必须用 branch，不要用 parallel。\n" +
  "- parallel：并行分支（同时执行多臂），尽可能不使用；仅当用户明确需要「并行/同时执行多条独立子流程」时才用。≥2 出边，label 为臂名称（如「扣库存」「发券」，禁纯数字/分支N）。各臂下游须汇合到同一后续节点（汇合点宜用 step，文案体现汇总/汇合）。\n" +
  ...
  "edges 每项：id,from,to,label（step/start/end 出发可为空；branch/parallel 出发 label 必填）。\n" +
  "nodes 每项：id,type,text,x,y；type=end 时加 outcome。\n" +
  "优先生成节点少、可读性高的图；默认用 branch 表达条件；关键失败路径用 end outcome=failure 建模。";
