...
export function buildIncrementalWorkflowPrompt(prompt, workflow, previousPrompt = "") {
  ...
  return [
    ...
    "条件分支用 branch；仅用户明确要求并行时用 parallel，且各臂须汇合到同一后续节点。"
  ].join("\n\n");
}

const PSEUDO_INCR_SYSTEM = `你是 MWGL v3 单节点伪代码增量优化器。

## 任务
根据该节点现有 .pseudo 正文、工作流与用户修改意见，输出更新后的节点正文（描述「做什么」）。

## 规则
- 只输出该节点 .pseudo 正文，不要 main.flow、不要 CALL、不要文件头
- 有 loop 的 step：仅一行 STEP 点明循环语义；FOR/WHILE 由程序追加
- 不要 markdown、不要 JSON`;
